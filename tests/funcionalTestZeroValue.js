...
(async function buttonRender() {
  ...
  try {
    ...
    let serviceValue = driver.findElement(By.id("valor-servico"));
    serviceValue.sendKeys(-1000);

    await driver.findElement(By.id('calcular')).click();

    let information = await driver.switchTo().alert().getText();
    await driver.switchTo().alert().accept();
    console.log("Test Zero Value - " + information);


    
    // Capture another screenshot after clicking the button
    const screenshotData2 = await driver.takeScreenshot();
    fs.writeFileSync('infs-ZeroValue.png', screenshotData2, 'base64');
    
    console.log('Screenshot Zero Value Changed saved.');
